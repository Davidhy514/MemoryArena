...
def init_search_engine(num_products=None):
    if num_products == 100:
        ...
        indexes = 'indexes_1k'
    elif num_products == 1275:
        indexes = 'indexes_1275'
    elif num_products == 100000:
        indexes = 'indexes_100k'
    elif num_products == 1181436:
    ...
    search_engine = LuceneSearcher(os.path.join(SEARCH_ENGINE_ROOT, indexes))
...
def load_products(filepath, num_products=None, human_goals=True):
    ...
    for i, p in tqdm(enumerate(products), total=len(products)):
        ...
        # Load rating directly from product data (not from separate reviews.json)
        ...
        if human_goals:
            ...
            # products[i]['instruction_attributes'] = \
            #     attributes[asin].get('instruction_attributes', None)
            # TODO: Edited by my self
            attrs = attributes.get(asin, {})
            products[i]['instruction_text'] = attrs.get('instruction')
            products[i]['instruction_attributes'] = attrs.get('instruction_attributes')
